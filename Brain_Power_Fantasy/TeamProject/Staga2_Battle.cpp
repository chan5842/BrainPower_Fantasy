...
Stage2_Battle::Stage2_Battle()
{
	...
	// 변수들의 초기화
	{
		...
		// 문제들을 vector에 삽입한다.
		q.push_back(Question(1, L"9+9+8 = ?", L"Test", "26"));
		q.push_back(Question(1, L"6*6*2 = ?", L"Test", "72"));
		q.push_back(Question(1, L"9-10+2 = ?", L"Test", "1"));
		q.push_back(Question(2, L"겜프 교수님 성함은?", L"1)최명걸 2)최 3)명 4)걸", "1"));
		q.push_back(Question(1, L"2*2*2*2 = ?", L"Test", "16"));
		q.push_back(Question(1, L"5/1+2 = ?", L"Test", "7"));
		q.push_back(Question(1, L"3*3*3/9+2 = ?", L"Test", "5"));
		q.push_back(Question(2, L"색이 아닌 것은?", L"1)빨강 2)노랑 3)파랑 4)돌", "4"));
		q.push_back(Question(1, L"3+4 = ?", L"Test", "7"));
		q.push_back(Question(2, L"전자제품이 아닌 것은?", L"1)화투 2)휴대폰 3)삐삐 4)무전기", "1"));
		q.push_back(Question(2, L"C++ 자료형이 아닌 것은?", L"1)int 2)float 3)double 4)triple", "4"));
		q.push_back(Question(1, L"2! = ?", L"Test", "2"));
		q.push_back(Question(1, L"3^3 = ?", L"Test", "27"));
	...
	// Character
	{
		...
		tm->LoadTexture("Enemy", "../Resources/Stage2_enemy.png");
		tm->AddSpriteRect("Stage2_Battle_enemy", 0, 0, 350, 414);
	}
...
Stage2_Battle::~Stage2_Battle()
{
	...
	std::cout << "delete Stage2_Battle" << std::endl;
}

// 스테이지 제한시간 
void Stage2_Battle::UpdateTimeTexture(int ms)
{
	if (text_time != 0)
	{
	...
	if (input.ScanInput())
	{
		...
		if (Input::input_space != "")
		{
			if (q[cnt].Q_Update(Input::input_space.c_str())) // 정답
			{
				...
				if (R_IP == 0)
				{
					g_current_game_phase = PHASE_STAGE2_END;
				}
				else
				{
					cnt++; // 다음 문제로 이동
...
void Stage2_Battle::Render()
{
	...
	// character
	{
		...
		tm->DrawSprite("Enemy", "Stage2_Battle_enemy", 850, 315);
	}
	// Effect
...
void Stage2_Battle::HandleEvents()
{
	input.HandleEvents();
}